...
//            mAuthTask.execute((Void) null);
    ...
    private void LoginMethod() {
        showProgress(true);
        loginApiImpl.loginAppRequest(email, password)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<BaseEntity<LoginEntity>>() {
                    @Override
                    public void accept(BaseEntity<LoginEntity> loginEntityBaseEntity) throws Exception {
                        showProgress(false);
                        if (loginEntityBaseEntity.getResponse().getError() != 0) {
                            toast(loginEntityBaseEntity.getResponse().getError_msg());
                        } else {
                            goMain(loginEntityBaseEntity.getResponse(), email, password);
                        }
                    }
